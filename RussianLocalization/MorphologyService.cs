...
namespace RussianLocalization
{
    ...
    public static class MorphologyService
    {
        ...
        private static readonly HashSet<string> HeadStopWords = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            // Держать в синхроне с WordFallbackPrepCases в RussianLocalization.cs: иначе
            // Decline примет предлог за главное существительное пары и просклоняет его
            // («глубокой кромю»). "кроме"/"против" вдобавок есть в stem_dictionary как основы.
        ...
        public static string Decline(string nominative, MorphCase targetCase, MorphNumber number = MorphNumber.Singular)
        {
            ...
            if (baseNominative.Contains(" ") || baseNominative.Contains("-"))
            {
                ...
                // 2026-08-25: фраза, НАЧИНАЮЩАЯСЯ с предлога/союза («с руками», «из кабаньей
                // кожи»), — предложная группа, а не именная: склонять в ней нечего, а поиск
                // головы ниже гарантированно ошибается (стоп-слово обрывает поиск сразу, и
                // запасная ветка берёт головой последнее слово: «с руками» -> Nom.Sg -> «с
                // рука» — так «two-handed», разрезанный рендерером по дефису, превращался в
                // «два-с рука»). Возвращаем как есть.
                for (int i = 0; i < rawParts.Length; i++)
                {
                    ...
                    if (HeadStopWords.Contains(first)) return nominative;
                    break;
                }
